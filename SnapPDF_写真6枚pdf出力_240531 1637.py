...
# 240531 写真6枚。タイトルと画像をpdf出力する。ページ番号表示。複数フォルダから画像選択。
# -------------------------------------------------------------
...
def create_pdf():
    ...
    for i, file_path in enumerate(image_paths):
        ...
        new_width = available_width / 3 - 10  # 3列で表示、間にスペース
        ...
        # 6枚の画像が集まったら、テーブルを作成してcontentに追加
        if len(image_table_data) == 6:
            content.append(Table([image_table_data[:3], file_name_table_data[:3]], colWidths=[available_width / 3] * 3))  # 画像とファイル名のテーブルを追加
