...
# 240613 Outputs 6 photos per page to pdf. You can select images from multiple folders.
# -------------------------------------------------------------
...
def create_pdf():
    ...
    for i, file_path in enumerate(image_paths):
        ...
        if len(image_table_data) == 6:
            content.append(Table([image_table_data[:3], file_name_table_data[:3]], colWidths=[available_width / 3] * 3))  # Add image and file name table
    ...
    if image_table_data:
        # Add the remaining image and file name table
        content.append(Table([image_table_data[:3], file_name_table_data[:3]], colWidths=[available_width / 3] * 3))
        content.append(Spacer(1, 0.1))  # Add space between image and file name
