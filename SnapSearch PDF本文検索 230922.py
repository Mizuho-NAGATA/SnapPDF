...
def search_pdf():
    ...
    if not search_results:
        ...
        # 検索結果のファイル名と場所を表示
        for filename, location in search_results:
            result_text = f'ファイル名: {filename}, 場所: {location}'
...
# ディレクトリ選択ボタン
select_directory_button = tk.Button(window, text='ディレクトリを選択', font=("Helvetica", 14), command=search_pdf)
select_directory_button.pack()

# CSV出力ボタンを追加
export_csv_button = tk.Button(window, text='ディレクトリを選択し検索結果をCSVに出力', font=("Helvetica", 14), command=search_pdf)
