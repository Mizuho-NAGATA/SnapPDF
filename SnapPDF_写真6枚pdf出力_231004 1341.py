...
def create_pdf():
    ...
    # 画像を表示
    max_width = 800  # リサイズ後の最大幅 (3枚の画像を横に並べた場合、1枚あたりの幅が800ピクセルになるように設定)
    max_height = 500  # リサイズ後の最大高さ (1枚の画像の高さが500ピクセルになるように設定)
    images_per_page = 3  # 1ページあたりの画像数を3に設定
    image_width = 260  # 画像の幅 (1行に3枚並ぶための幅)
    image_height = 500  # 画像の高さ (大きく表示するための高さ)
    ...
    doc = SimpleDocTemplate(pdf_file_path, pagesize=landscape(A4), topMargin=1.2 * inch, bottomMargin=0.1 * inch)  # ページ上下の余白
    ...
    def add_title_and_page_number(canvas, doc):
        ...
        remarks.drawOn(canvas, inch, A4[0] - inch * 1.2)  # 備考をページ上端からxインチ下に描画。
