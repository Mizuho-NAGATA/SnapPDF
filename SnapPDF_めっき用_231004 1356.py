...
# 231004 Excelファイルを読み込み、選択した画像をファイル名と共にpdf出力するプログラム。偶数行に背景色。
# 画像を複数のフォルダから追加
# -------------------------------------------------------------
...
def select_excel_file():
    ...
    file_path = filedialog.askopenfilename(filetypes=[("Excel files", "*.xlsx *.xls")], title="めっきExcelファイルを選択") # ダイアログボックスのタイトルを設定
    if file_path:
        # Excelファイルからデータを読み込む
        try:
...
def create_pdf():
    ...
    doc = SimpleDocTemplate(pdf_file_path, pagesize=landscape(A4), topMargin=1.5 * inch, bottomMargin=0.15 * inch)  # ページ上下の余白
    content = []
    ...
    # 0行目と偶数行の背景色を追加 (9行目まで)
    for row in range(0, min(data_table._nrows, 10), 2):
        table_style.append(('BACKGROUND', (0, row), (-1, row), (0.8, 0.9, 1.0)))  # 背景色を設定
