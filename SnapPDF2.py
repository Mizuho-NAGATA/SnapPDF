...
# 240531 1ページに写真2枚をpdf出力。複数フォルダから画像選択することができます。
# -------------------------------------------------------------
...
def create_pdf():
    ...
    for i, file_path in enumerate(image_paths):
        ...
        # 画像がページに収まるかどうかを確認
        if new_height > available_height - 10:  # 1行で表示、間にスペース
            new_height = available_height - 10  # 1行で表示、間にスペース
        ...
        if len(image_table_data) == 2:
            content.append(Table([image_table_data, file_name_table_data], colWidths=[available_width / 2] * 2))  # 画像とファイル名のテーブルを追加
