...
# 240613 Outputs 2 photos per page to pdf. You can select images from multiple folders.
# -------------------------------------------------------------
...
def create_pdf():
    ...
    for i, file_path in enumerate(image_paths):
        ...
        if new_height > available_height - 10:  # Display in 1 row, with space in between
            new_height = available_height - 10  # Display in 1 row, with space in between
        ...
        if len(image_table_data) == 2:
            content.append(Table([image_table_data, file_name_table_data], colWidths=[available_width / 2] * 2))  # Add image and file name table
