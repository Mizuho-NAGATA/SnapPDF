...
def process_image_for_pdf(file_path):
    ...
    new_width = (
        A4[1] - 2 * inch
    ) / 2 - 10  # Display in 2 columns, with space in between
    new_height = new_width / image_ratio

    # Check if the image fits on the page
    if new_height > (
        A4[0] - 2.5 * inch - 0.5 * inch - 10
    ):  # Display in 1 row, with space in between
...
def create_pdf():
    ...
    for image, name in results:
        ...
        if len(image_table_data) == 2:
            content.append(
                Table(
                    [image_table_data, file_name_table_data],
                    colWidths=[(A4[1] - 2 * inch) / 2] * 2,
                )
            )  # Add image and file name table
            content.append(
                Spacer(1, 0.1)
            )  # Add minimal space between image and file name
            image_table_data = []
            file_name_table_data = []
    ...
    if image_table_data:
        content.append(
            Table(
                [image_table_data, file_name_table_data],
                colWidths=[(A4[1] - 2 * inch) / 2] * len(image_table_data),
            )
        )
